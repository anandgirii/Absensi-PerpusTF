...
def face_recognition():  # Generate frame by frame from camera
    global camera
    if 'camera' not in globals() or camera is None or not camera.isOpened():
            ...
            if confidence > 75 and not justscanned:
                ...
                if row:
                    ...
                    if not detection_complete:
                        ...
                        if capture_count >= 20:  # Setelah mencapai 20 gambar
...
def enable_camera():
    ...
    if camera is None or not camera.isOpened():
        camera = cv2.VideoCapture(0)
    return jsonify({"status": "Camera enabled"}), 200
...
def disable_camera():
    ...
    if camera is not None and camera.isOpened():
        ...
        cv2.destroyAllWindows()
        camera = None
    return jsonify({"status": "Camera disabled"}), 200
...
def loadData():
    ...
    mycursor.execute("""
        SELECT a.accs_id, a.accs_prsn, b.prs_name, a.status, 
    ...
    # Hitung jumlah IN dan OUT tanpa duplikasi
    mycursor.execute("""
        SELECT COUNT(DISTINCT accs_prsn)
        FROM accs_hist
